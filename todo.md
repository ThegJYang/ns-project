...
- [x] Flagship elasticity theorem ρ(S) = nₖ/n₁ proved, zero sorries (14 theorems)
- [x] Mathematical foundations — `Defs.lean`, `Factorization.lean`
...
- [ ] Prove `frobeniusNumber` correct against `Generated`, connecting `Defs.lean` to `Basic.lean`
...
`Defs.lean` and `Basic.lean` are unconnected. Lean proves `frobeniusNumber [5,13,21] = 37`, which is a fact about our code rather than about the semigroup. GAP agreement is strong empirical evidence, but the formal guarantee we advertise does not yet reach the mathematics.
